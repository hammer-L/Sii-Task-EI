...
def prepocess_input_sequence(
    ...
    """
    把输入的 str 转化为 ids，即 text to ids
    """
    out = []
...
def scaled_dot_product_no_loop_batch(
    ...
    weights = torch.bmm(query, key.transpose(1, 2)) / (M ** 0.5)
    if mask is not None:
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        q_transformed = self.q(query)
        k_transformed = self.k(key)    # (N, K, dim_q)
        v_transformed = self.v(value)  # (N, K, dim_v)

        y, self.weights_softmax = scaled_dot_product_no_loop_batch(q_transformed, k_transformed, v_transformed, mask)
        return y
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        y = self.output_proj(concatenated)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        self.hidden_dim_feedforward = hidden_dim_feedforward
...
def get_subsequent_mask(seq):
    ...
    # Replace "pass" statement with your code
    N, K = seq.shape
    subsequent_mask = torch.triu(torch.ones(K, K), diagonal=1)
    mask = subsequent_mask.unsqueeze(0).expand(N, K, K).bool()
...
class DecoderBlock(nn.Module):
    ...
    def forward(
        ...
        out1 = self.norm1(attn1 + dec_inp)
        out1 = self.dropout(out1)

        attn2 = self.attention_cross(out1, enc_inp, enc_inp)
        out2 = self.norm2(attn2 + out1 )
        out2 = self.dropout(out2)

        ff_out = self.feed_forward(out2)
        y = self.norm3(ff_out + out2)
        y = self.dropout(y)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    positions = torch.arange(0, K, dtype=torch.float32)
    position_encodings = positions / K
    position_encodings = position_encodings.unsqueeze(0).unsqueeze(-1)
    y = position_encodings.expand(1, K, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        mask = get_subsequent_mask(ans_b[:, :-1]).to(ans_b.device)
        dec_out = self.decoder(a_emb_inp, enc_out, mask)
        dec_out = dec_out.reshape(-1, M)
        ...
        dec_out = F.softmax(dec_out.detach(), dim=-1)
