...
def train(lr, vocab_size, batch_size, epochs, model_type="transformer"):
    ...
    for epoch in tqdm(range(epochs)):
        ...
        if (epoch+1)%200==0:
            if model_type == "transformer":
                train_acc = evaluate(train_x, train_y, model, 200)
                test_acc = evaluate(test_x, test_y, model,  200)
            else:
        ...
        if (epoch + 1) % 2500 == 0:
            torch.save(model.state_dict(),f"ckpts/{model_type}_lr={lr}_epoch_{epoch+1}.pt")
